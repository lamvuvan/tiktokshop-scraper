...
# opts.add_experimental_option('webdriver.chrome.driver', '/usr/bin/chromedriver')
...
def scrape(category, limit):
    ...
    for i in range(1, limit + 1):
        try:
            username = wait(browser, 10).until(EC.presence_of_element_located(
                (By.XPATH, f'(//div[@class="flex items-center"]/div[@elementtiming="element-timing"])[{i}]')))
...
def open_browser(category, limit, datas):
    datas = datas.split("|")
    email = datas[0]
    password = datas[1]
    global browser
    ...
    try:
        browser.get(
            'https://seller-id.tiktok.com/account/login?redirect_url=https%3A%2F%2Faffiliate.tiktok.com%2Fseller%2Fdashboard')
        sleep(5)
    except:
        sleep(2)
